...
WITH
  ...
  (
    select
      ...
      and med.gmrs_cd='G',0.12,0.18)									intr_icms_rt, --D19 = D45 Ok
  ...
  (
    select cte_med_st_3.*,
      trans_icms_rt * (1 - icms_reductor_rt)																			reduced_icms_rt, --D35 Ok
  ...
  (
    select cte_med_st_5.*,
      if(seller_is_manuf_ind = 0 and trans_is_intra_st_ind = 0, 
    pcm_prc_amt, 
    mva_calc
  )																														calc_st
    --D44 Ok
