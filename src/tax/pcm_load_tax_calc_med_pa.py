 ...
 with
      ...
      (
        select
          ...
          MED.mva_7_pct,
          MED.mva_4_pct,
          if(TRANS.seller_is_manuf_ind = 0 and trans_is_intra_st_ind = 1, 0.40, MED.mva_orig_pct) mva_orig_pct, -- D43
          DPROD.pck_is_medcn_ind,
          if(TRANS.seller_is_manuf_ind = 0 and trans_is_intra_st_ind = 1, 0.5294, MED.red_mva_pct/100 ) red_mva_rt, --45
          if(TRANS.seller_is_manuf_ind = 0 and trans_is_intra_st_ind = 1, 0, coalesce(MED.pcm_red_pct, 0) /100) pcm_red_rt, -- D40
          med.gmrs_cd,DPROD.pack_pnn_flag_cd,trans.prc_disc_pct
      ...
      (
        select
          ...
          if(seller_is_manuf_ind = 0 and trans_is_intra_st_ind = 1, 0.52941, 0.2941) red_intr --D35
        from cte_med_st_1
      ),
      cte_med_st_3
      as
      ...
      (
        select
          cte_med_st_3.*,
          trans_icms_rt * (1-icms_reductor_rt)*(1-red_intr) icms_reduced, --D36
      ...
      (
        select
          ...
          0, 
          fnal_purchase_price_amt * icms_reduced
        ) icms_own_amt, -- D36 
      ...
      (
        select
          cte_med_st_5.*,
          if(seller_is_manuf_ind = 0 and trans_is_intra_st_ind = 1, 
          mva_calc,
          if(pcm_prc_amt = 0,
              mva_calc,
              reduced_pmc
