...
spark.sql(f"""
    ...
    select
        ...
        and med.gmrs_cd='G',0.12,0.18)									intr_icms_rt, --D19 = D52
        ...
        coalesce(pr_c.prc_amt/100, 0) pmpf_prc_amt, -- D38
        0																	cst_cd,
        trans.dsupp_is_distr_ind											dsupp_is_distr_ind,
        med.pnn_cd															pnn_cd,
        coalesce(PR_M.prc_amt, 0) pcm_prc_amt, -- D38
        med.trava_pct / 100												trava_rt, --D42			
        med.mva_12_pct,
        med.mva_7_pct,
        med.mva_4_pct,
        med.mva_orig_pct , --D46
  ...
  (
    select cte_med_st_9.*,
        fnal_purchase_price_amt + st_prc - pis_cofins_cr_amt	ppp_prc_amt  --D55
    from cte_med_st_9
  )
